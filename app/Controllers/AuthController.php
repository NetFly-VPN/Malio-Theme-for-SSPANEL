        ...
        if (Config::get('enable_email_verify') == true) {
            ...
            try {
                Mail::send($email, $subject, 'auth/verify.tpl', [
                    'code' => $code, 'reply_to' => Config::get('smtp_replyTo'), 'expire' => date('Y-m-d H:i:s', time() + Config::get('email_verify_ttl'))
                ], [
            ...
            /*
{
    ...
    public function registerWithDeviceHandle($request, $response)
    {
        if (Config::get('register_mode') === 'close') {
            $res['ret'] = 0;
            $res['msg'] = '未开放注册。';
        ...
        */
        ...
        $user->im_value = $antiXss->xss_clean($wechat);
        // 设备注册账户没有流量
        $user->transfer_enable = 0;//Tools::toGB(Config::get('defaultTraffic'));
        $user->invite_num = Config::get('inviteNum');
        $user->auto_reset_day = Config::get('reg_auto_reset_day');
        $user->auto_reset_bandwidth = Config::get('reg_auto_reset_bandwidth');
        $user->money = 0;
